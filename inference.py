...
def constructBayesNet(gameState: hunters.GameState):
    ...
    for i in variables:
        if i == PAC or i == GHOST0 or i == GHOST1:
            pos = [(x, y) for x in range(X_RANGE) for y in range(Y_RANGE)]
            variableDomainsDict[i] = pos
        else:
            obs = set()
            for pos1 in pos:
                for pos2 in pos:
                    dist = manhattanDistance(pos1, pos2)
                    for n in range(-MAX_NOISE, MAX_NOISE + 1):
                        if dist + n >= 0:
                            obs.add(n + dist)
                        
            variableDomainsDict[i] = list(obs)

    "*** END YOUR CODE HERE ***"
    print(variables, edges, variableDomainsDict)
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for var in eliminationOrder:
            factors, newfact = joinFactorsByVariable(factors, var)
            if len(newfact.unconditionedVariables()) <= 1:
                continue
            else:
                factors.append(eliminate(newfact, var))
...
class DiscreteDistribution(dict):
    ...
    def normalize(self):
        ...
        "*** YOUR CODE HERE ***"
        totalSum = self.total()
        if not self or totalSum == 0:
            return
        for key, val in self.items():
            self[key] = val / totalSum
        "*** END YOUR CODE HERE ***"
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        for ghost_pos, val in self.beliefs.items():
            p = self.getObservationProb(observation, gameState.getPacmanPosition(), ghost_pos, self.getJailPosition())
            self.beliefs[ghost_pos] = val * p
        "*** END YOUR CODE HERE ***"
        self.beliefs.normalize()
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for oldpos in self.allPositions:
            newdict = self.getPositionDistribution(gameState, oldpos)
            original_val = self.beliefs[oldpos]
            for newpos in self.allPositions:  
                newbeliefs[newpos] += original_val * newdict[newpos]
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for _ in range(self.numParticles):
            new_particles.append(dist.sample())
